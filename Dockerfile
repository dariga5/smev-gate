...
&& mv ~/bpm-service-unixbuild $ADAPTER_DIR 

COPY ./entrypoint.sh $ADAPTER_DIR
RUN chown -R smev:smev $ADAPTER_DIR 

USER smev
WORKDIR $ADAPTER_DIR
RUN mkdir -p $ADAPTER_DIR/data/in
